...
import os
import json
from pulp import LpProblem, LpMinimize, LpVariable, lpSum, LpBinary, LpInteger, PULP_CBC_CMD
...
prob += lpSum(buy_listing_vars[i] * listing_map[i]['price'] for i in listing_map) + \
        lpSum(use_seller_vars[s] * get_shipping_cost_for_seller(s) for s in sellers)
...
for s in sellers:
    ...
    for i in indices:
        prob += buy_listing_vars[i] <= listing_map[i]['quantity'] * use_seller_vars[s]
...
for i, var in buy_listing_vars.items():
    ...
    if qty > 0:
        ...
        row_data['effective_total'] = row_data['price'] * qty + row_data['shipping_cost']
...
if not df_cart.empty:
    ...
    total_card_price = (df_cart['price'] * df_cart['quantity']).sum()
    total_shipping = sum(get_shipping_cost_for_seller(seller) for seller in sellers_used)
    final_total = total_card_price + total_shipping

    print_colored(f"\nTotal card price: ${total_card_price:.2f}", "green")
    print_colored(f"Shipping cost ({len(sellers_used)} sellers): ${total_shipping:.2f}", "green")
    ...
    print_colored("\nCart saved to 'optimized_mtg_cart_sorted.csv' (sorted by seller and quantity)", "blue")
